...
public class CardDisplay : MonoBehaviour
{
    ...
    public int cardIndex;               //손패에서 인덱스 (나중에 사용)
    ...
    private void OnMouseDrag()
    {
        if (isDragging)
        {
            ...
            Vector3 worIdPos = Camera.main.ScreenToWorldPoint(mousePos);
            transform.position = new Vector3(worIdPos.x, worIdPos.y, transform.position.z);
        }
    }
    private void OnMouseUp()
    {
        ...
        //여기서 부터 카드 사용로직 (마나체크)
        CaracterStats playerStats = null;
        GameObject player0bj = GameObject.FindGameObjectWithTag("Player");
        if (player0bj != null)
        {
            playerStats = player0bj.GetComponent<CaracterStats>();
        }
        ...
        {
            CaracterStats enemyStats = hit.collider.GetComponent<CaracterStats>();        //적에게 공격 효과 적용
            if (enemyStats != null)
            {
    ...
    private void ProcessAdditionalEffectsAndDiscard()
    {
        ...
        foreach (var effect in cardDataCopy.additionalEffects)
        {
            ...
            {
                case CardData.AdditionalEffectType.DrawCard:                    //도로우 카드 구현
                ...
                case CardData.AdditionalEffectType.DiscardCard:         //카드 버리기 구연(랜덤 버러기)
                    for (int i = 0; i < effect.effectAmount; i++)
                    {
                        if(cardManager != null && cardManager.handCards.Count > 0)
                        {
                            int randomlndex = Random.Range(0, cardManager.handCards.Count); //손패 크기 기준으로 랜덤인덱스 생성

                            Debug.Log($"랜덤 카드 버리기 : 선택된 인덱스 {randomlndex}, 현재 손패 크기 : {cardManager.handCards.Count}");

                            if(cardlndexCopy < cardManager.handCards.Count)
                            {
                                if(randomlndex != cardlndexCopy)
                                {
                                    cardManager.DiscardCard(randomlndex);

                                    //만약 버린 카드의 인덱스가 현재 카드의 인덱스 보다 작다면 현재 카드의 인덱스를 1 감소시켜야함
                                    if (randomlndex < cardlndexCopy)
                                    {
                                        cardlndexCopy--;
                                    }
                                ...
                                {
                                    //다른 카드 선택
                                    int newlndex = (randomlndex + 1) % cardManager.handCards.Count;
                                    cardManager.DiscardCard(newlndex);
                            ...
                            {
                                //cardlndexCopy 가 더이상 유효가지 않은 경우 , 아무 카드나 버림
                                cardManager.DiscardCard(randomlndex);
                            }
                        }
                    }
                ...
                case CardData.AdditionalEffectType.GainMana:    //플레이어 마나 획득
                    ...
                    if (playerObj != null)
                    {
                        CaracterStats playerStats = playerObj.GetComponent<CaracterStats>();
                        if (playerStats != null)
                        {
                ...
                case CardData.AdditionalEffectType.ReduceEnemyMana:     //적 마나 감소
                    GameObject[] enemies = GameObject.FindGameObjectsWithTag("Ebentg");//태그를 사용하여 플레이어 캐릭터찾기
                    foreach (var enemy in enemies)
                    {
                        CaracterStats enemyStats = enemy.GetComponent<CaracterStats>();
                        if (enemyStats != null)
                        {
        ...
        //효과 적용 후 현재 카드 버리기
        if (cardManager != null)
            cardManager.DiscardCard(cardlndexCopy);
    }
