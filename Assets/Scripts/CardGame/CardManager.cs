...
public class CardManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.F))                //F키를 누르면 버린 카드를 덱으로 되돌라고 섞기
        {
    ...
    {
        List<CardData> tempDack = new List<CardData>(deckCards);        //임시 리스트에 카드 복사
        deckCards.Clear();

        //랜덤하게 섞기
        while(tempDack.Count > 0)
        {
            int randIndex = Random.Range(0, tempDack.Count);
            deckCards.Add(tempDack[randIndex]);
            tempDack.RemoveAt(randIndex);
        }
    ...
    public void ArrangeHand()
    {
        ...
        for(int i = 0; i < card0bjects.Count; i++)
        {
            if(card0bjects[i] != null)
            {
                ...
                //부드러운 이동
                card0bjects[i].transform.position = Vector3.Lerp(card0bjects[i].transform.position, targetPosition, Time.deltaTime * 10f);
            }
        }
    }
    ...
    public void DiscardCard(int handIndex)          //카드 버리기 (디스 카드)
    {
        ...
        discardCards.Add(cardData);             //버리는 카드 더미에 추가

        discardCards.Add(cardData);
        if (handIndex < card0bjects.Count)          //해당 카드 게임 오브젝트 제거
        {
            Destroy(card0bjects[handIndex]);
            card0bjects.RemoveAt(handIndex);
        }
        for (int i = 0; i < card0bjects.Count; i++) //카드 인덱스 재설정
        {
            CardDisplay display = card0bjects[i].GetComponent<CardDisplay>();
