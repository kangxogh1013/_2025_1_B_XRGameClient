...
public class DialogManager : MonoBehaviour
{
    ...
    [SerializeField] private Image portraitlmage;                        //캐릭터 초상화 이미지UI 요소 추가
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError("Dialog Daatabase is not assinged to Dialog Manager");
        }
        if (NextButton != null)
        {
    ...
    {
        DialogSO dialog = dialogDatabase.GetDialogById(dialogld);
        if (dialog != null)
        {
            StartDialog(dialog);
        }
    ...
    public void ShowDialog()
    {
        ...
        {
            portraitlmage.sprite = currentDialog.portrait;
            portraitlmage.gameObject.SetActive(true);
        }
        else if (!string.IsNullOrEmpty(currentDialog.portraitPath))
        {
            ...
            if(portrait != null)
            {
                portraitlmage.sprite = portrait;
                portraitlmage.gameObject.SetActive(true);
            }
            else
            {
                Debug.LogWarning($"Portrait not found at path : {currentDialog.portraitPath}");
                portraitlmage.gameObject.SetActive(false);
            }
        }
        else
        {
            portraitlmage.gameObject.SetActive(false);                        //초상화가 없으면 이미지 비활성화
        }
